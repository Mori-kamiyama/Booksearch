  ...
  return (
    <div className="bg-white pb-24 md:pb-0">
      <div className="mx-auto w-full max-w-[402px] px-7 pt-[49px] md:max-w-none md:px-0 md:pt-0">
        <div className="hidden md:mx-auto md:mb-[39px] md:mt-[54px] md:block md:w-[920px]">
          <SearchBar value={query} onChange={setQuery} onSubmit={runSearch} />
        </div>

        <div className="md:mx-auto md:w-[920px]">
          ...
          <section className="grid gap-8 md:grid-cols-[300px_1fr] md:gap-[44px]">
  ...
  return (
    <section className="flex flex-col items-center md:items-start">
      <div className="relative flex h-[350px] w-full max-w-[240px] items-end justify-center md:h-[405px] md:max-w-[300px]">
        {cover ? (
          <img src={cover} alt="" className="h-full w-auto max-w-full object-contain" />
        ) : (
          <div className="grid h-full w-full place-items-center bg-[#d9d9d9] text-ink-muted">
...
function BookInfoPanel({ book, readingHours }: { book: Book; readingHours: number }) {
  return (
...
function detailUrlForBook(book: Book): string {
  if (book.info_link) return book.info_link
      ...
      {candidate ? (
        <div className="mt-4 flex flex-col items-center gap-4 md:mt-6 md:grid md:grid-cols-[1fr_355px] md:items-center md:gap-[44px]">
          <div className="order-2 text-center md:order-1 md:text-left">
            ...
            <p className="mt-2 text-xs text-ink-muted">観測 {candidate.observations}回 / 信頼度 {Math.round(candidate.confidence * 100)}%</p>
...
function AiSummarySection({ text }: { text: string }) {
  return (
    <section className="mx-auto mt-12 w-full max-w-[402px] px-7 md:mt-16 md:max-w-[886px] md:px-0">
      <h2 className="text-base font-semibold leading-[19px] text-ink">AIカルテ</h2>
      <div className="mt-4 text-sm leading-7 text-ink md:mt-5 md:text-base">
        <p className="font-semibold">内容要約</p>
...
function Recommendations({ books, onOpen }: { books: Book[]; onOpen: (bookId: number) => void }) {
  ...
  return (
    <section>
      <h2 className="text-lg font-bold leading-[1.4] text-ink">この本を読むあなたに</h2>
      <div className="-mx-7 mt-5 flex snap-x snap-mandatory scroll-px-7 items-start gap-6 overflow-x-auto px-7 pb-2 [-webkit-overflow-scrolling:touch] [scrollbar-width:none] md:mx-0 md:snap-none md:gap-5 md:overflow-visible md:px-0 [&::-webkit-scrollbar]:hidden">
        {books.map(book => {
          const cover = book.thumbnail || fallbackCoverForTitle(book.title)
          return (
            <button
            ...
            >
              <div className="flex h-[155px] w-[114px] items-end justify-center md:h-[150px] md:w-[112px]">
                {cover ? (
                  ...
                  <div className="grid h-full w-full place-items-center bg-[#d9d9d9] text-ink-muted"><BookOpen className="size-6" /></div>
                )}
              </div>
...
function DetailSkeleton() {
  return (
    <div className="mx-auto grid max-w-[402px] gap-8 px-7 pt-12 md:max-w-[920px] md:grid-cols-[300px_1fr] md:px-0">
...
function recommendationsForDisplay(books: Book[]): Book[] {
  ...
  return fallbackRecommendationBooks()
}
