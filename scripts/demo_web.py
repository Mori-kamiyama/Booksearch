...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--apriltag-map", default=None, help="AprilTag→棚IDマッピングJSON")
    parser.add_argument("--max-tag-distance", type=float, default=None, help="tag検出最大距離px")
    ...
    if isbns:
        ...
        return {
            "thumbnail": (
                ...
                f"vid=ISBN{urllib.parse.quote(isbn)}&printsec=frontcover&img=1&zoom=1&source=gbs_api"
            ),
            "info_link": f"https://books.google.com/books?vid=ISBN{urllib.parse.quote(isbn)}",
            "title": title,
            "source": "google_books_content",
        }

    cache = load_cover_cache()
    key = f"title:{title or ''}"
    if key in cache:
        return cache[key]
...
def process_image(
    ...
    # 動画なら先にフレーム抽出
    if upload_path.suffix.lower() in VIDEO_EXTENSIONS:
        frames_dir = run_dir / "frames"
        images = book_video.extract_frames(upload_path, frames_dir, interval_sec=2.0)
        if not images:
            raise RuntimeError(f"動画からフレームを抽出できませんでした: {upload_path}")
        is_video = True
    else:
        images = [upload_path]
        is_video = False
    ...
    # AprilTag 棚割り当て
    shelf_assignments: dict[str, Any] = {}
    if apriltag_map and apriltag_map.exists():
        boxes_by_image: dict[Path, list] = {}
        for box in boxes:
            boxes_by_image.setdefault(box.image_path, []).append(box)
        for img_path, img_boxes in boxes_by_image.items():
            assignments, _ = shelf_locator.locate_shelves_for_boxes(
                image_path=img_path,
                boxes=img_boxes,
                mapping_path=apriltag_map,
        ...
        for book in books:
            ...
            if candidate.get("thumbnail"):
                cover = {
                    ...
                    "info_link": candidate.get("info_link"),
                    "title": candidate.get("title") or book.get("title"),
                    "source": "library_db",
                }
            else:
    ...
    preview_path = run_dir / "previews" / images[0].name if images else None
    result = {
        "input": upload_path,
        "run_dir": run_dir,
        "preview": preview_path if (preview_path and preview_path.exists()) else None,
        "is_video": is_video,
        "frame_count": len(images) if is_video else 1,
        "entries": entries,
    }
...
def upload_form(message: str = "") -> bytes:
    ...
    <p style="color:var(--muted);font-size:12px;margin-top:6px">画像（JPG/PNG）または動画（MP4/MOV）をアップロードできます。</p>
    {note}
    """
...
def render_result(result: dict[str, Any]) -> bytes:
    ...
    for entry in entries:
        ...
        if entry.get("shelf_id"):
            shelf_badge = f' · <span style="color:var(--accent);font-weight:700">棚: {html.escape(entry["shelf_id"])}</span>'
        sections.append(
            f"""
            <section class="box">
              <div>
                <h2>{html.escape(entry["box_id"])}{shelf_badge} · {len(entry["books"])} books</h2>
    ...
    if result.get("is_video"):
        frame_info = f'<div class="metric"><strong>{result.get("frame_count", 0)}</strong><span>frames</span></div>'
    body = f"""
    <form class="upload" method="post" enctype="multipart/form-data" action="/process">
      <input type="file" name="image" accept="image/*,video/*" required>
      <button type="submit">別の画像/動画を解析</button>
    </form>
    <div class="summary">
...
class DemoHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        upload_path = upload_dir / f"upload_{int(time.time())}{suffix}"
        upload_path.write_bytes(field.file.read())

        apriltag_map = None
        if self.args.apriltag_map:
            apriltag_map = resolve_path(self.args.apriltag_map)
        ...
        try:
            result = process_image(
                ...
                max_tag_distance=self.args.max_tag_distance,
            )
        except Exception as exc:
