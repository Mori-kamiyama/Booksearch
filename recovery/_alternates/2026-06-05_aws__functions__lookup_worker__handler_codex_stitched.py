...
def process_job(job_id: str) -> None:
    ...
    jobs_table.update_item(
        Key={"job_id": job_id},
        UpdateExpression="SET #s = :s, catalog_key = :k, updated_at = :t",
...
def handler(event, context):
    for rec in event.get("Records", []):
        ...
        try:
            ...
            raise
    return {"ok": True}
    scores = [float(row.get("evidence_score") or 0) for row in rows]
    avg_score = sum(scores) / observations
    confidence = avg_score * (1 - math.pow(0.72, observations))
    confidence = min(0.99, confidence)
    last_seen_at = max((row.get("observed_at") or "") for row in rows) or fallback_seen_at
    ...
    shelf_candidates_table.put_item(
        Item={
            ...
            "registration_number": candidate.get("registration_number") or "",
            "isbn": (candidate.get("isbns") or [""])[0],
...
def process_job(job_id: str) -> None:
    catalog = build_catalog(job_id)
    observations_added = update_shelf_confidence_from_catalog(job_id, catalog)
    key = f"catalogs/{job_id}/catalog.json"
    s3.put_object(
        Bucket=BUCKET, Key=key,
        Body=json.dumps(catalog, ensure_ascii=False, indent=2).encode("utf-8"),
    ...
    jobs_table.update_item(
        ...
        ExpressionAttributeNames={"#s": "status"},
        ExpressionAttributeValues={
            ":s": "done", ":k": key, ":n": observations_added,
            ":t": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        },
        UpdateExpression="SET #s = :s, catalog_key = :k, shelf_observations_added = :n, updated_at = :t",
    )
    print(f"[lookup] done {job_id} -> {key} shelf_observations_added={observations_added}")
