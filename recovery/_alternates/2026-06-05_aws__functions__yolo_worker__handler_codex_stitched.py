...
処理:
  ...
  7. jobs テーブル更新: OCR 対象 crop 数、status = "ocr_pending"
  8. readable な crop に対して OCR queue に投入
"""
...
def process_job(job_id: str, image_key: str) -> None:
    ...
    jobs_table.update_item(
        Key={"job_id": job_id},
        UpdateExpression="SET #s = :s, crop_total = :ocr_n, detected_crop_total = :det_n, "
                         "skipped_crop_total = :skip_n, ocr_done = :z, "
                         "image_width = :w, image_height = :h",
        ExpressionAttributeNames={"#s": "status"},
        ExpressionAttributeValues={
            ":s": "ocr_pending" if readable_count > 0 else "no_readable_crops",
            ":ocr_n": readable_count,
            ":det_n": len(crop_records),
            ":skip_n": len(crop_records) - readable_count,
            ":z": 0,
            ":w": width,
            ":h": height,
        },
