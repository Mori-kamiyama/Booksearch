...
画像アップロードから、箱検出、crop、Gemini OCR、ローカル図書 DB 検索、Google Books 書影表示まで確認できるデモ UI を作成済み。
...
発表前には、成功しやすい画像を 1-3 枚固定し、その画像で毎回同じ流れを説明できるようにする。

## 現時点で次にやること

アノテーションと学習は完了済みなので、次は追加学習ではなく、成果物として見せる部分を固める。

優先度:

1. デモで成功しやすい画像を選ぶ。
2. `demo_web.py` で結果を確認する。
3. 成功例の preview と `result.json` を発表用に固定する。
4. 3 枚サンプルの評価値を発表資料に載せる。
5. 失敗例を 1 つだけ用意し、今後の改善点として説明する。

詳細な現状まとめは `docs/project_status.md` に置いた。
